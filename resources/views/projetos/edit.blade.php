                    ...
                    <form method="POST"
                            ...
                            <select id="setor_id" name="setores_id"
                                ...
                                @foreach($setores as $setor)
                                    <option value="{{ $setor->id }}"
                                        @selected(old('setores_id', $projeto->setores_id) == $setor->id)>
                                        {{ $setor->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="prazo"
                                          ...
                                          type="date"
                                          class="mt-1 block w-full focus:border-verdes-verde_claro focus:ring-verdes-verde_claro"
                                          :value="old('prazo', $projeto->prazo?->format('Y-m-d'))" />
                        ...
                        <div>
                            ...
                            <x-text-input id="orcamento_previsto"
                                          ...
                                          step="0.01"
                                          class="mt-1 block w-full focus:border-verdes-verde_claro focus:ring-verdes-verde_claro"
                                          :value="old('orcamento_previsto', $projeto->orcamento_previsto)"
                                          placeholder="0,00" />
                        ...
                        <div>
                            ...
                            <x-text-input id="orcamento_real"
                                          ...
                                          step="0.01"
                                          class="mt-1 block w-full focus:border-verdes-verde_claro focus:ring-verdes-verde_claro"
                                          :value="old('orcamento_real', $projeto->orcamento_real)"
                                          placeholder="0,00" />
                        ...
                        <div>
                            ...
                            @php $statusValue = old('status', $projeto->status); @endphp
                            <select id="status" name="status"
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900
                                           text-sm text-gray-900 dark:text-gray-100 focus:border-verdes-verde_claro focus:ring-verdes-verde_claro">
                                <option value="aberto" @selected($statusValue === 'aberto')>Aberto</option>
                                <option value="em_andamento" @selected($statusValue === 'em_andamento')>Em andamento</option>
                                <option value="concluido" @selected($statusValue === 'concluido')>Concluído</option>
                        ...
                        <div>
                            ...
                            <textarea id="descricao" name="descricao" rows="4"
                                      class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900
                                             text-sm text-gray-900 dark:text-gray-100 focus:border-verdes-verde_claro focus:ring-verdes-verde_claro">{{ old('descricao', $projeto->descricao) }}</textarea>
                        ...
                        <div>
                            ...
                            {{-- Erros dos arquivos de orçamento (orcamentos.*) --}}
                            @php
                                $errosOrcamentos = collect($errors->get('orcamentos.*'))->flatten()->toArray();
                            @endphp

                            <x-input-error :messages="$errosOrcamentos" class="mt-2" />
                        </div>

                        {{-- Anexos atuais --}}
                        @if($projeto->orcamentos && $projeto->orcamentos->count())
                            <div class="border-t border-gray-200 dark:border-gray-700 pt-3">
                                <p class="text-xs uppercase text-gray-500 dark:text-gray-400 font-semibold mb-2">Anexos atuais</p>
                                <ul class="space-y-2 text-sm">
                                    @foreach($projeto->orcamentos as $arquivo)
                                        @php $url = $arquivo->path ? asset('storage/'.$arquivo->path) : '#'; @endphp
